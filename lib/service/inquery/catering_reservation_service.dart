...
class CateringReservationService {
  Future<BanquetReservationListResponse> fetchReservations() async {
    final token = await getToken();

    try {
      final response = await DioClient.dio.get(
        '/customer/catering-enquiry/reservations/all',
      ...
      if (kDebugMode) {
        print('Catering reservation fetch error: $e');
      }
      return BanquetReservationListResponse(
        status: false,
  ...
  Future<BanquetReservationListResponse> searchReservations(String id) async {
    ...
    try {
      final response = await DioClient.dio.get(
        '/customer/catering-enquiry/reservations/search',
      ...
      if (kDebugMode) {
        print('Catering reservation search error: $e');
      }
      return BanquetReservationListResponse(
        status: false,
        message: 'Something went wrong: $e',
  ...
  Future<Map<String, dynamic>> cancelReservation(String reservationId) async {
    ...
    try {
      final response = await DioClient.dio.post(
        '/customer/catering-enquiry/reservation/cancel',
      ...
      if (kDebugMode) {
        print('Catering reservation cancel error: $e');
      }
